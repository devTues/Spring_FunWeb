...
<c:forEach var="dto" items="${boardListMap }">
<tr onclick="location.href='${pageContext.request.contextPath }/board/content?num=${dto.num}'">
    <td>${dto.boardNum}</td>
    <td class="left">${dto.boardSubject}</td>
    <td>${dto.boardName}</td>
    <td><fmt:formatDate value="${dto.boardDate}" pattern="yyy.MM.dd"/></td>
    <td>${dto.boardReadcount}</td></tr> 
</c:forEach>
